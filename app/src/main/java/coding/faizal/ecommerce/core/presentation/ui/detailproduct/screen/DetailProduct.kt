package coding.faizal.ecommerce.core.presentation.ui.detailproduct.screen
...
import coding.faizal.ecommerce.core.domain.model.local.product.ProductImage
...
import coding.faizal.ecommerce.presentation.ui.checkout.screen.CheckoutActivity
import coding.faizal.ecommerce.presentation.ui.detailproduct.adapter.ProductColorAdapter
import coding.faizal.ecommerce.presentation.ui.detailproduct.adapter.ProductSizeAdapter
import coding.faizal.ecommerce.presentation.ui.detailproduct.adapter.ViewPagerAdapter
import coding.faizal.ecommerce.core.presentation.ui.detailproduct.utils.CircleAnimationUtils
...
import coding.faizal.ecommerce.presentation.ui.home.utils.Utils
...
class DetailProduct : AppCompatActivity() {
    ...
    private var productImage = mutableListOf<coding.faizal.ecommerce.core.domain.model.local.product.ProductImage>()
    ...
    private fun setupView(i : coding.faizal.ecommerce.core.domain.model.remote.product.Product){
        binding.apply {
            ...
            i.variants.mapIndexed { index, productVariant ->
                if(index < i.imgSrc.size){
                    ...
                    productImage.add(
                        coding.faizal.ecommerce.core.domain.model.local.product.ProductImage(
                            index + 1,
            ...
            btnBuy.setOnClickListener {
                startActivity(Intent(this@DetailProduct, CheckoutActivity::class.java).also{
                    it.putExtra(DETAIL, OrderItemRequest(i._id,"Small",1,i.price))
                })
            }
    ...
    private fun makeFlyAnimation(imageView : ImageView) {
        ...
        coding.faizal.ecommerce.core.presentation.ui.detailproduct.utils.CircleAnimationUtils().attachActivity(this).setTargetView(imageView).setMoveDuration(1000)
    ...
    private fun seeAll(i : coding.faizal.ecommerce.core.domain.model.remote.product.Product){
        binding.seAllSize.setOnClickListener { showBottomSheetBuy(i) }
        binding.seeAllColor.setOnClickListener { showBottomSheetBuy(i) }
    }

    private fun showBottomSheetBuy(i : coding.faizal.ecommerce.core.domain.model.remote.product.Product){
    ...
    private fun setProductColorRecycler(){
        ...
        productColorAdapter = ProductColorAdapter(this@DetailProduct,productImage,object : ProductColorAdapter.ProductImageOnClick{
            override fun productImage(productImage: coding.faizal.ecommerce.core.domain.model.local.product.ProductImage, position : Int) {
    ...
    private fun setProductSizeRecycler(datas : List<String>,i : coding.faizal.ecommerce.core.domain.model.remote.product.Product){
        lifecycleScope.launch {
            productViewModel.productSizeResult.collect{ data ->
                val x : MutableList<coding.faizal.ecommerce.core.domain.model.local.product.ProductSize> = mutableListOf()
                    data.mapIndexed { index, productSize ->
                     if(data.size <= datas.size){
                         x.add(
                             coding.faizal.ecommerce.core.domain.model.local.product.ProductSize(
                                 productSize.id,
                                 datas[index],
                                 false
                             )
                ...
                productSizeAdapter = ProductSizeAdapter(this@DetailProduct,x.distinct(),object : ProductSizeAdapter.ProductSizeOnClick{
                    override fun productSize(productSize: coding.faizal.ecommerce.core.domain.model.local.product.ProductSize, position: Int) {
        ...
        authPreferencesViewModel.getToken().observe(this@DetailProduct){
            if(it != null){
                productViewModel.getProductSize(it)
            }
        }
    ...
    private fun bottomNavBuy(i : coding.faizal.ecommerce.core.domain.model.remote.product.Product){
    ...
    private fun bottomNavDescription(i : coding.faizal.ecommerce.core.domain.model.remote.product.Product) {
