...
import coding.faizal.ecommerce.databinding.FragmentCategoryBinding
...
@AndroidEntryPoint
class CategoryFragmentShirtFormal : Fragment() {
            ...
            productViewModel.listProductResult.collect{ resource ->
                when (resource) {
                    ...
                    is coding.faizal.ecommerce.core.data.Resource.Success -> {
                        ...
                        if(result != null){
                            val finalResult = result.filter { it.name.contains("kemeja",ignoreCase = false) && it.name.contains(i,ignoreCase = false) }
