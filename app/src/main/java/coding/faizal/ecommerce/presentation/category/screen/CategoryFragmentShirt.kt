...
import coding.faizal.ecommerce.databinding.FragmentCategoryKidsBinding
...
@AndroidEntryPoint
class CategoryFragmentShirt : Fragment() {


    private var _binding : FragmentCategoryKidsBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentCategoryKidsBinding.inflate(layoutInflater)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (bundle != null) {
            val nilaiData = bundle.getString("category")
           if(nilaiData != null){
               Toast.makeText(requireActivity(), "$nilaiData", Toast.LENGTH_SHORT).show()
               getAllProduct(nilaiData)
           }
        }
            ...
            productViewModel.listProductResult.collect{ resource ->
                when (resource) {
                    ...
                    is Resource.Success -> {
                        ...
                        if(result != null){
                            val finalResult = result.filter { it.name.contains("kaos",ignoreCase = false) && it.name.contains(i,ignoreCase = false) }
